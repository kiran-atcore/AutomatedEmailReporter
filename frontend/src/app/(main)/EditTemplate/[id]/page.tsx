    ...
    const fetchTemplate = async () => {
      try {
        ...
        setTemplateData({
          ...
          css_overrides: response.data.css_overrides || "",
          has_chart: response.data.has_chart || false,
          chart_type: response.data.chart_type || "bar",
          email_subject: response.data.email_subject || "",
          email_body_html: response.data.email_body_html || "",
          enable_ai_summary: response.data.enable_ai_summary || false,
          ai_prompt: response.data.ai_prompt || "",
          branding_color: response.data.branding_color || "#1e3c72"
        });
        if (response.data.branding_logo) {
          setLogoPreview(response.data.branding_logo);
        }
      } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (logoFile) {
        ...
        fd.append('branding_logo', '');
      }
      
      await api.put(`/reports/templates/${id}/`, fd, {
          ...
          <div className="col-lg-8">
            ...
            >
              ...
              <div className="mb-5 position-relative z-1 d-flex align-items-center gap-3">
                ...
                <div>
                  <h3 className="fw-bolder mb-1" style={{ fontFamily: "var(--font-righteous)", background: "linear-gradient(135deg, #ffffff 0%, #a1a1aa 100%)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", fontSize: "2rem", letterSpacing: "0.5px" }}>Edit Template</h3>
                  <p className="theme-text-muted mb-0" style={{ fontSize: "1.1rem" }}>Update the structure and layout for your automated PDFs.</p>
                </div>
              </div>
